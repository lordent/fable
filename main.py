...
from concept.models import Users, Shops, Cities, Categories, Sales
from sql.db import Config, Engine
from sql.select import Select, JoinStrategy
from sql.query import List, Item
from sql.core import Rank, Sum, Count, concat
from sql.field import DecimalField
...
async def _load_data():
    ...
    async with Users.atomic() as conn:
...
async def run_test():
    ...
    users = await Select(Users.id, Users.name).limit(5)
    for row in users:
        print(f"User: {row['id']} - {row['name']}")

    print("\n--- Тест 2. Поиск по триграммам (Advisor check) ---")
    pro_users = await Select(Users.name).filter(Users.name.similar('Александр'))
    print(f"Найдено похожих: {len(pro_users)}")
    ...
    sub = Select(Categories.id, Categories.name).filter(Categories.parent_id == 2).as_table()
    query = Select(Categories.name, sub_name=sub.name).join(sub, on=(Categories.parent_id == sub.id))
    ...
    res = await query
    for row in res:
        print(f"{row['name']}: {int(row['age'])} лет (Родился: {int(row['birth_month'])}/{int(row['birth_year'])})")
    ...
    share = (Sales.amount / category_total * 100).cast(DecimalField(5, 2))
    ...
    print("-" * 50)
    for row in res:
        print(f"{row['category_name']:<15} | {row['amount']:<8} | {row['rank']:<5} | {row['share']}%")
    ...
    query = (
        Select(
            ...
            children_count=Count(Categories.id)
        )
        .join(
            Parent, 
            on=(Categories.parent_id == Parent.id), 
            strategy=JoinStrategy.RIGHT
        )
